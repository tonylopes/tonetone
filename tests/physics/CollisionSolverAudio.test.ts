...
// test here — not the synthesis, which `tests/audio/Voices.test.ts` covers.
//
// This used to mock the whole audio module and read the choice back out of the
// call arguments. The solver no longer calls the voices: it records a SoundEvent,
// so the choice is now plain data on `state.sounds` and no mock is needed.
import { collide, CollisionState } from '../../src/physics/CollisionSolver';
import { recalcThresholds } from '../../src/physics/Config';
import { Ball, SoundEvent } from '../../src/physics/Types';
...
/** The recorded sounds of one type, narrowed to that member of the union. */
function soundsOf<T extends SoundEvent['type']>(
  state: CollisionState,
  type: T
): Extract<SoundEvent, { type: T }>[] {
  return state.sounds.filter((s): s is Extract<SoundEvent, { type: T }> => s.type === type);
}

/** The `whiteBlack` flag each boom recorded. */
function boomVariants(state: CollisionState): boolean[] {
  return soundsOf(state, 'boom').map(s => s.whiteBlack);
}

/** Whether each magnet lock joined two black balls. */
function magnetVariants(state: CollisionState): boolean[] {
  return soundsOf(state, 'magnetLock').map(s => s.bothBlack);
}

/** The group size each magnet lock was scaled by. */
function magnetSizes(state: CollisionState): number[] {
  return soundsOf(state, 'magnetLock').map(s => s.size);
}
...
describe('CollisionSolver voice selection', () => {
  ...
  describe('white-on-black boom', () => {
    it('gives the boom its own boom when a white destroys a black', () => {
      ...
      expect(boomVariants(state)).toEqual([true]);
    });
    ...
    it('leaves a white boom on colours alone with the ordinary boom', () => {
      ...
      expect(boomVariants(state)).toEqual([false]);
    });
    ...
    it('keeps the ordinary boom when a colour booms a group the black survives', () => {
      ...
      for (const v of boomVariants(state)) expect(v).toBe(false);
    });
  });
  ...
  describe('magnet lock', () => {
    it('rings a black locking to a black higher than a black locking to a colour', () => {
      ...
      expect(magnetVariants(state)).toEqual([false]);
    });
    ...
    it('scales the lock by the group the merge produced, not by either side', () => {
      ...
      expect(bRed.bonds.has(1)).toBe(true);
      expect(magnetSizes(state)).toEqual([members.length + 1]);
    });
    ...
    it('marks a black-on-black lock as the pair variant', () => {
      ...
      expect(magnetVariants(state)).toEqual([true]);
    });
  });
});
